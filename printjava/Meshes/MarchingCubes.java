...
public class MarchingCubes extends Mesh {
    ...
    private final Function<Point, Double> field;
        ...
        for (int i = 0; i < nx; i++)
            for (int j = 0; j < ny; j++)
                for (int k = 0; k < nz; k++) {
                    ...
                    values[i][j][k] = field.apply(new Point(x, y, z));
                }
    }
            ...
            for (int j = 0; j < ny - 1; j++)
                for (int k = 0; k < nz - 1; k++) {
                    ...
                    for (int n = 0; n < 8; n++) {
                        ...
                        double x = xmin + (i + xo) * dx;
                        double y = ymin + (j + yo) * dy;
                        double z = zmin + (k + zo) * dz;

                        p[n] = new Point(x, y, z);
                        v[n] = values[i + xo][j + yo][k + zo];
                    }

                    int cubeIndex = 0;
                    for (int n = 0; n < 8; n++)
                        if (v[n] < iso) cubeIndex |= 1 << n;

                    int edgeMask = EDGE_TABLE[cubeIndex];
                    if (edgeMask == 0) continue;

                    Point[] edgeVerts = new Point[12];

                    for (int e = 0; e < 12; e++) {
                        if ((edgeMask & (1 << e)) != 0) {
                            int a = EDGE_VERTICES[e][0];
                            int b = EDGE_VERTICES[e][1];
                            edgeVerts[e] = interpolate(p[a], p[b], v[a], v[b]);
                        }
                    }

                    int[] table = TRIANGLE_TABLE[cubeIndex];
                    for (int t = 0; table[t] != -1; t += 3) {
                        add(new Triangle(
                            edgeVerts[table[t]],
                            edgeVerts[table[t + 1]],
                            edgeVerts[table[t + 2]]
    ...
    private Point interpolate(Point a, Point b, double va, double vb) {
        if (Math.abs(iso - va) < 1e-6) return a;
        if (Math.abs(iso - vb) < 1e-6) return b;
        if (Math.abs(va - vb) < 1e-6) return a;
